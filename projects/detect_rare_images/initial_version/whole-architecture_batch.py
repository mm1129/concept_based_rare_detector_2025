...
def detect_new_images(
    ...
    if len(features_new) == 0:
        ...
        return
    # only_additional_candidatesの場合はbaseline_featuresがないので処理を分岐
    if only_additional_candidates or baseline_features is None:
        # baseline_featuresがない場合は、features_newだけでt-SNEを計算
        print("ベースライン特徴がないため、新しい特徴のみでt-SNEを計算します")
        tsne = TSNE(n_components=2, random_state=42, perplexity=min(30, len(features_new)-1))
        new_tsne = tsne.fit_transform(features_new)
        ...
        if iso_model is not None:
            ...
            for code in [0, 1]:
                indices_code = np.where(if_outliers == code)[0]
            ...
            plt.title("新規サンプルのt-SNE可視化 (IsolationForest)")
            plt.legend(loc='best')
            plt.tight_layout()
            tsne_path = os.path.join(main_out_dir, "new_samples_tsne.png")
            plt.savefig(tsne_path)
            plt.close()
            from collections import Counter
            # クラス別の可視化を追加
            new_class_names = [info[1] for info in all_cropped_info]
            class_counts = Counter(new_class_names)
            ...
            for i, cls in enumerate(top_classes):
                indices = [j for j, name in enumerate(new_class_names) if name == cls]
                if indices:
                    plt.scatter(new_tsne[indices, 0], new_tsne[indices, 1], 
                               label=f"{cls} ({len(indices)})", 
                               color=cmap(i/len(top_classes)), 
                               alpha=0.7)
            
            plt.title("新規サンプルのt-SNE可視化 (クラス別)")
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.tight_layout()
            tsne_class_path = os.path.join(main_out_dir, "new_samples_tsne_class.png")
            plt.savefig(tsne_class_path, bbox_inches='tight')
            plt.close()
            
            # 異常スコアを計算
            combined_outlier_code = if_outliers
        else:
            # IsolationForestモデルがない場合は全てインライアとする
            combined_outlier_code = np.zeros(len(features_new), dtype=int)
            
            # IsolationForestモデルがなくても、クラス別の可視化は行う
            new_class_names = [info[1] for info in all_cropped_info]
            try:
                from collections import Counter
                class_counts = Counter(new_class_names)
                ...
                print(f"クラス別t-SNE可視化エラー: {e}")
    else:
        # 通常の処理：ベースライン特徴と新しい特徴を組み合わせる
...
def main():
    ...
    if args.split_mode == "standard":
        # only_additional_candidates が True の場合はベースラインモデルのトレーニングをスキップ
        if args.only_additional_candidates:
            print("only_additional_candidates=True のため、ベースラインモデルのトレーニングをスキップします")
            # CLIPエクストラクタのみ初期化
            clip_extractor = CLIPFeatureExtractor(model_name="openai/clip-vit-base-patch32")
            # 他の変数はダミー値を設定
            candidate_labels = concept_list
            baseline_features = np.zeros((1, 512))  # ダミーの特徴ベクトル
            baseline_tsne = np.zeros((1, 2))  # ダミーのt-SNE
            class_names = []
            iso_model = None
        else:
            # 従来の方法でベースラインモデルを学習
            ...
            if baseline_features is None and not args.only_additional_candidates:
                return
