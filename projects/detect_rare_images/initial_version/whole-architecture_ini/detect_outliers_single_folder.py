...
try:
    ...
    print("OpenAI API key not found. Set OPENAI_API_KEY in .env file. or you can coninue with laion/CLIP-ViT-B-32")
...
def generate_descriptions_batch(image_paths, model, processor, batch_size=8, detected_classes=None):
    ...
    for i, path in enumerate(image_paths):
        ...
        is_rare = any(keyword in detected_class.lower() for keyword in 
                     ["construction", "bicycle", "motorcycle", "trailer", "truck", "fence", "barrier", "traffic_cone", "traffic_light", "traffic_sign"])
    ...
    # 希少クラスのみキャプション生成
    print(f"キャプション生成: 全{len(image_paths)}枚中{len(rare_class_paths)}枚を詳細処理")
    ...
    try:
        ...
        for i, path in enumerate(rare_class_paths):
            ...
            if i % 10 == 0 or i == len(rare_class_paths) - 1:
                ...
                print(f"希少クラスキャプション生成: {progress:.1f}% ({i+1}/{total_images}) | 経過: {elapsed:.1f}秒 | 残り: {eta:.1f}秒")
            ...
            try:
                ...
                # YOLOで検出されたクラス名を取得
                detected_class = detected_classes.get(path, "") if detected_classes else ""
                class_hint = f" Note that YOLO says the image is about a {detected_class} but it can be wrong." if detected_class else ""
                ...
                try:
                    ...
                    with torch.cuda.amp.autocast(enabled=True):
                        generated_ids = model.generate(
                            **inputs,
                            max_new_tokens=150,
                            do_sample=True,      # 警告を消すためサンプリングを有効化
                            top_p=1.0,           # 固定値で警告を解消
                            top_k=50,            # 必要に応じて調整
                            num_beams=1,
                            temperature=1.0
                    ...
                    # トリミング時に長さをチェックし、安全にスライス
                    trimmed = []
                    for in_ids, out_ids in zip(inputs.input_ids, generated_ids):
                        in_len  = in_ids.shape[0]
                        out_len = out_ids.shape[0]
                        if out_len <= in_len:
                            # 生成結果がなくなってしまった場合
                            trimmed.append(None)
                        else:
                            trimmed.append(out_ids[in_len:])
                    ...
                    for ids in trimmed:
                        if ids is None or ids.numel() == 0:
                            ...
                            decoded.append(processor.decode(
                                ids, skip_special_tokens=True, clean_up_tokenization_spaces=False
                            ))
                    rare_results.append(decoded[0] if decoded else None)
                except Exception as e:
    ...
    for i in range(len(image_paths)):
        if i not in rare_class_indices:
            if non_rare_idx < len(results) - len(rare_class_indices):
...
def detect_outliers_single_folder(
    ...
    Args:
        ...
        skip_visualization: t-SNE可視化をスキップするかどうか
        skip_caption: キャプション生成をスキップするかどうか
        parallel: 並列処理を有効にするかどうか
        workers: 並列処理のワーカー数（指定しない場合は自動設定）
    """
    # シグナルハンドラを設定
    ...
    if sampling_rate < 1.0:
        ...
        print(f"サンプリング適用: {orig_count}枚 → {len(all_files)}枚 (サンプリング率: {sampling_rate:.1%})")
    ...
    # 早期フィルタリング: YOLOの検出結果から直接希少クラスを判定
    ...
    print(f"早期フィルタリング: {len(all_cropped_info)}個 → {len(filtered_cropped_info)}個のオブジェクト")
    ...
    # 希少クラスフィルタリングも併用
    class_names = ['truck', 'bicycle', 'motorcycle', 'trailer', 'construction_vehicle']
    rare_indices = filter_rare_classes(valid_cropped_info, class_names)
    
    # 処理対象のインデックスを決定（アウトライアと希少クラス）
    process_indices = list(set(np.where(outlier_flags == 1)[0]).union(set(rare_indices)))
    process_indices.sort()  # ソートして順序を保つ
    
    print(f"処理対象オブジェクト数: {len(process_indices)} (アウトライア: {np.sum(outlier_flags)}, 希少クラス: {len(rare_indices)})")
    ...
    # if not skip_caption:
    try:
        print(f"Qwen2VLモデルを初期化しています (サイズ: {qwen_model_size})...")
        ...
        # キャプションをスキップする場合はダミーのモデルとプロセッサを作成
        model = None
        processor = None
        print("キャプション生成をスキップするためモデルは初期化しません")
    # 13. バッチ処理でキャプション生成
    ...
    if process_paths:
        ...
        print("キャプション生成をスキップします")
        # 簡易判定のみ実行
        for i in process_indices:
            info = valid_cropped_info[i]
            class_name = info[1]
            is_final_outlier = is_potential_rare_class(class_name)
            # ...結果保存...
...
def map_to_target_class(concept_name, target_classes, similarity_threshold=0.7):
    """
    コンセプト名を類似する希少クラス名にマッピングする関数
    例: 'bulldozer' → 'construction_vehicle'
    """
    # 希少クラスのマッピング辞書
    class_mappings = {
        "construction_vehicle": [
            ...
            "construction", "digger", "loader", "grader", "paver"
        ],
        "bicycle": [
    ...
    # 直接一致するターゲットクラスがあれば、そのまま返す
    if concept_lower in [tc.lower() for tc in target_classes]:
        return concept_name
    
    # 各ターゲットクラスについて、関連キーワードとの一致を確認
...
def main():
    ...
    parser.add_argument("--sampling_rate", type=float, default=0.2, 
                        help="処理する画像のサンプリング率 (0.0-1.0)")
    parser.add_argument("--skip_visualization", action="store_true", default=True,
                        help="t-SNE可視化をスキップする")
    parser.add_argument("--skip_caption", action="store_false", 
                        help="詳細なキャプション生成を実行する（デフォルトはスキップ）")
    parser.add_argument("--yolo_model", type=str, default="yolov8l.pt",
                        choices=["yolov8s.pt", "yolov8m.pt", "yolov8l.pt", "yolov8x.pt"],
                        help="使用するYOLOモデル（大きいほど精度が高いが遅い）")
    parser.add_argument("--clip_model", type=str, 
                        default="laion/CLIP-ViT-B-32-laion2B-s34B-b79K",
                        choices=[
                            "openai/clip-vit-base-patch32",  # 軽量・高速（デフォルト）
                            "openai/clip-vit-base-patch16",  # バランス型
                            "openai/clip-vit-large-patch14", # 高精度・低速
                            "laion/CLIP-ViT-B-32-laion2B-s34B-b79K"  # LAION版（オープンデータ学習）
                        ],
                        help="使用するCLIPモデル")
